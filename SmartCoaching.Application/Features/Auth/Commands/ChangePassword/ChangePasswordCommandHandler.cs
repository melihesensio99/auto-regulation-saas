...
public class ChangePasswordCommandHandler : IRequestHandler<ChangePasswordCommand, Result<bool>>
{
    ...
    public async Task<Result<bool>> Handle(ChangePasswordCommand request, CancellationToken cancellationToken)
    {
        ...
        {
            return Result.Failure<bool>(new Error("Auth.Unauthorized", "Geçerli bir kullanıcı bulunamadı.", ErrorType.Unauthorized));
        }

        var athlete = await _context.Athletes.IgnoreQueryFilters().SingleOrDefaultAsync(a => a.Id == userId, cancellationToken);
        if (athlete == null)
        {
            return Result.Failure<bool>(new Error("Auth.UserNotFound", "Kullanıcı bulunamadı.", ErrorType.NotFound));
        }

        bool isOldPasswordValid = _passwordHasher.Verify(request.OldPassword, athlete.PasswordHash);
        if (!isOldPasswordValid)
        {
            return Result.Failure<bool>(new Error("Auth.InvalidOldPassword", "Eski şifre hatalı.", ErrorType.Validation));
        }
